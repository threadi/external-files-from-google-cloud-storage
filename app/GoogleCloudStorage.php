...
namespace ExternalFilesFromGoogleCloudStorage;
...
class GoogleCloudStorage extends Service_Base implements Service {
	...
	public function init_google_cloud_storage(): void {
		...
		// add the section for file statistics.
	...
	private function get_authentication_json(): string {
		...
		if ( $this->is_mode( 'user' ) ) {
			...
			// get the value.
			return Crypt::get_instance()->decrypt( get_user_meta( $user->ID, 'efml_google_cloud_storage_json', true ) ); // @phpstan-ignore class.notFound
		}
	...
	public function get_bucket_name(): string {
		...
		if ( $this->is_mode( 'user' ) ) {
			...
			// return the value.
			return Crypt::get_instance()->decrypt( get_user_meta( $user->ID, 'efml_google_cloud_storage_bucket', true ) ); // @phpstan-ignore class.notFound
		}
	...
	public function get_directory_listing( string $directory ): array {
		...
		try {
			foreach ( $bucket->objects() as $file_obj ) {
				 ...
				 */
				if ( apply_filters( 'efmlgcs_service_googlecloudstorage_hide_file', $false, $file_data, $directory ) ) {
					continue;
				}
	...
	public function add_protocol( array $protocols ): array {
		...
		array_unshift( $protocols, 'ExternalFilesFromGoogleCloudStorage\GoogleCloudStorage\Protocol' );
	...
	public function get_user_settings(): array {
		 ...
		 */
		return apply_filters( 'efmlgcs_service_google_cloud_user_settings', $list );
	}
	...
	public function get_public_url_for_file( string $bucket_name, string $file_name ): string {
		 ...
		 * @param string $file_name The file name.
		 */
		return apply_filters( 'efmlgcs_service_google_cloud_storage_public_url', $url, $bucket_name, $file_name );
	}
	...
	private function get_console_url(): string {
		 ...
		 * @param string $url The URL.
		 */
		return apply_filters( 'efmlgcs_service_google_cloud_storage_console_url', $url );
	}
